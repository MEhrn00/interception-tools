...
struct cmd {
    ...
    std::vector<pid_t> launch() const {
        ...
        for (size_t i = 0; i < cmds.size(); ++i) {
            ...
            case 0: {
                std::unique_ptr<char *[]> command {new char *[cmds[i].size() + 1]};
...
struct job {
    job(const YAML::Node& job_node, const YAML::Node& settings_doc = {}) {
        ...
        if (auto driver_version = device["DRIVER_VERSION"])
            this->driver_version.assign(driver_version.as<string>(), regex::optimize);
        ...
        if (auto properties = device["PROPERTIES"]) {
            for (const auto& property_node : properties) {
                ...
                for (const auto& property_name : property_names) {
                    ...
                    if (property < 0)
                        throw invalid_argument("invalid EVENT CODE: " + property_name);
        ...
        if (auto events = device["EVENTS"]) {
            for (const auto& event : events) {
                ...
                int event_type =
                    is_int(event_type_name)
                        ? stoi(event_type_name)
                        : libevdev_event_type_from_name(event_type_name.c_str());
                if (event_type < 0)
                    throw invalid_argument("invalid EVENT TYPE: " + event_type_name);
    ...
    next:
        ...
        if (!regex_match(to_string(libevdev_get_id_product(e)), product) ||
            ...
            !regex_match(to_string(libevdev_get_driver_version(e)), driver_version))
            return false;

        if (!properties.empty() &&
            none_of(
                properties.begin(), properties.end(), [e](const vector<int>& property) {
                    return all_of(property.begin(), property.end(), [e](int property) {
                        return libevdev_has_property(e, property);
                    });
                }))
            return false;

        return all_of(events.begin(), events.end(),
    ...
    std::vector<pid_t> launch_for(const std::string& devnode) const {
        ...
        for (size_t i = 0; i < cmds.size(); ++i) {
            ...
            case 0: {
                ...
                char *environment[] = {const_cast<char *>(variables.c_str()), nullptr};
                setpgid(0, 0);
                execvpe(command[0], command.get(), environment);
...
struct jobs_manager {
        ...
        for (const auto& config : configs)
            ...
            case 2:
                if (config[0].IsSequence() == config[1].IsSequence())
                    throw invalid_argument("configuration must contain one job node's "
                                           "sequence document");
                ...
                break;
            default:
                throw invalid_argument("unexpected number of documents in configuration");
                break;
            }
    }
    ...
    void launch_for(udev_device *u) {
        ...
        if (!devnode || std::strncmp(devnode, input_prefix, sizeof(input_prefix) - 1))
            return;
...
std::vector<yaml> scan_config(const std::string& directory) {
    static const std::regex yaml_extension {R"(.*\.ya?ml)", std::regex::optimize};
    ...
    if (DIR *dir = opendir(directory.c_str()))
        while (dirent *entry = readdir(dir))
            if ((entry->d_type == DT_REG || entry->d_type == DT_LNK) &&
                ...
                configs.push_back(YAML::LoadAllFromFile(directory + '/' + entry->d_name));
...
int main(int argc, char *argv[]) try {
    ...
    if (configs.size() > 0)
        printf("%zu configuration files read from /etc/interception/udevmon.d\n",
               configs.size());
    ...
    for (int opt; (opt = getopt(argc, argv, "hc:")) != -1;) {
        ...
        case 'c':
            try {
                ...
                if (std::regex_match(optarg, default_config) && configs.size() > 0)
                    continue;
