...
int main(int argc, char *argv[]) try {
    enum { NO_MODE, CREATE_MODE, INPUT_MODE, OUTPUT_MODE, SWITCH_MODE } mode = NO_MODE;
    ...
    for (int opt, last_opt = 0; (opt = getopt(argc, argv, "hs:c:i:o:")) != -1;) {
        switch (opt) {
        case 'h':
    ...
    case INPUT_MODE: {
        ...
        for (;;) {
            ...
            if (size != sizeof input)
                ...
                throw std::runtime_error("error writing input event to stdout");
        }
    } break;
    ...
    case SWITCH_MODE: {
        ...
        muxers.emplace_back();
        for (const auto& muxer_name : muxer_names[""])
            muxers.back().emplace_back(new message_queue(open_only, muxer_name.c_str()));
        ...
        for (const auto& muxer_name : muxer_names) {
            ...
            for (const auto& name : muxer_name.second)
                muxers.back().emplace_back(new message_queue(open_only, name.c_str()));
