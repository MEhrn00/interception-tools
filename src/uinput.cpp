...
bool bit(const block_type buffer[], int bit_index) {
    return buffer[bit_index / block_size] & (block_type {1} << (bit_index % block_size));
}
...
std::string yaml_create_from_evdev(libevdev *dev) {
    ...
    if (auto bustype = libevdev_get_id_bustype(dev)) {
        if (bus_string.find(bustype) != bus_string.end())
            yaml << YAML::Key << "BUSTYPE" << YAML::Value << bus_string[bustype];
        else
    ...
    if (ioctl(fd, EVIOCGBIT(0, EV_MAX), type_mask) != -1 &&
        ...
        for (int type_code = 0; type_code <= EV_MAX; ++type_code) {
            ...
            case EV_ABS:
                if (ioctl(fd, EVIOCGBIT(type_code, event_max), event_mask) != -1) {
                    yaml << YAML::BeginMap;
                    for (int event_code = 0; event_code <= event_max; ++event_code) {
                        if (!bit(event_mask, event_code))
                            continue;
                        if (auto absinfo = libevdev_get_abs_info(dev, event_code)) {
                            if (auto event_name =
                                    libevdev_event_code_get_name(type_code, event_code))
                                yaml << YAML::Key << event_name << YAML::Value;
                            else
                                yaml << YAML::Key << event_code << YAML::Value;
                            yaml << YAML::BeginMap;
                            yaml << YAML::Key << "VALUE" << YAML::Value << absinfo->value;
                            yaml << YAML::Key << "MIN" << YAML::Value << absinfo->minimum;
                            yaml << YAML::Key << "MAX" << YAML::Value << absinfo->maximum;
                            if (absinfo->flat > 0)
    ...
    for (const auto& config : configs) {
        ...
        if (auto property = config["PROPERTIES"])
            for (auto it = property.begin(); it != property.end(); ++it) {
                auto property = libevdev_property_from_name(it->as<string>().c_str());
                if (property != -1)
            ...
            for (const auto& event_type : event_types) {
                ...
                if (event_type_string == "EV_REP") {
                    ...
                    for (const auto& axis : event_type.second) {
                        ...
                        if (axis_code != -1)
                            libevdev_enable_event_code(dev, EV_ABS, axis_code, &absinfo);
                    }
                } else {
                    auto event_type_code =
                        libevdev_event_type_from_name(event_type_string.c_str());
...
int main(int argc, char *argv[]) try {
    ...
    if (libevdev_uinput_create_from_device(dev, LIBEVDEV_UINPUT_OPEN_MANAGED, &uidev) < 0)
        return perror("libevdev_uinput_create_from_device failed"), EXIT_FAILURE;
    struct defer2 {
        libevdev_uinput *uidev;
        ~defer2() {
    ...
    while (fread(&input, sizeof input, 1, stdin) == 1)
        if (libevdev_uinput_write_event(uidev, input.type, input.code, input.value) < 0)
